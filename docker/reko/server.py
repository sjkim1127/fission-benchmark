...
def functions(binary: str):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        shutil.copy(resolved, tmp_binary)
        ...
        if not c_file:
            raise HTTPException(status_code=500, detail=f"No C output generated by Reko. Stderr: {result.stderr[:500]}")
        ...
        for addr_hex, name in matches:
            try:
                ...
                addr_formatted = f"0x{addr_int:x}"
                if addr_formatted not in seen:
                    seen.add(addr_formatted)
                    funcs.append({
