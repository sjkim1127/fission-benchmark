...
def test_run_parity_writes_mismatch_json(mock_glob, mock_fetch, tmp_path):
    ...
    def side_effect(file, *args, **kwargs):
        if str(file) == str(mock_manifest):
            # Return a mock open handle with manifest_data
    ...
    with patch("builtins.open", side_effect), \
         patch.dict("os.environ", {"PARITY_DIFFS_DIR": str(parity_diffs_dir)}):
